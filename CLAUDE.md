...
| `dcaExportSheet` | `DCAAgent.exportDCAToSheet(userId, month)` — reads `dca_plans`+`dca_plan_items` for the month and appends **one row per ticker per portfolio** (`Month·Portfolio·Ticker·Suggested·Planned·Actual·Done·Export Date`) + a per-month `ALL·TOTAL` summary row to the spreadsheet **`MyAsset+ DCA History`** (id remembered in Script Property `DCA_SHEET_ID`; created on first export via `_getOrCreateDCASheet`). Returns `result.export = {exported, month, sheetName, url, rows, done, total}`. Needs the `spreadsheets` + `drive.file` scopes in `gas/appsscript.json`. Backs the DCA page's **📊 Export to Google Sheets** button (frontend then stamps the month `submitted` + rolls to next month). Params `userId`, `month` (`YYYY-MM`). Replaced the old `dcaEmailSummary`/`emailMonthSummary` (GmailApp) action |
...
dca           DCA Plan — multi-portfolio manual planner. Month selector (+ "+" to add next month) + portfolio tabs; per tab: budget input + 🔄 Refresh (gap-from-target suggestions), a TICKER/Reasoning/Suggested/Planned/Actual/✓ table, status pill (DRAFT→IN PROGRESS→COMPLETED→SUBMITTED) + progress bar. **📊 Export to Google Sheets** appends the month to the `MyAsset+ DCA History` spreadsheet via GAS `dcaExportSheet` (opens the sheet url), then archives the month (`status:'submitted'`) and rolls the planner over to a fresh next month. Button block hides on already-submitted months.
...
- `_dcaUpdateCompleteBtn()` — queries all of the month's plans with embedded `dca_plan_items(is_done)`; renders the single **📊 Export to Google Sheets** button (when ≥1 item), and **hides the whole button block on already-submitted months** (`anySubmitted`); note text explains it exports to “MyAsset+ DCA History”, archives the month, and starts next month. `_dcaExportSheet()` → `callGAS('dcaExportSheet', {userId, month})`; on failure toasts `❌ Export ไม่สำเร็จ — กรุณา re-authorize GAS ก่อน`; on success toasts `✅ Exported to Google Sheets — MyAsset+ DCA History`, `window.open`s `res.export.url`, PATCHes the month's plans to `status:'submitted'`, inserts blank next-month plans for every portfolio (budget via `_dcaPriorBudget`), sets `_dcaMonth = next` and `loadDCA()`. (The old **📧 Submit & Email Summary**/`_dcaSubmitEmail` + **✅ Complete Month**/`_dcaCompleteMonth` were removed.)
