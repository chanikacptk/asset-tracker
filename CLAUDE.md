...
| `dcaEmailSummary` | `DCAAgent.emailMonthSummary(userId, month)` — builds a per-portfolio plain-text DCA summary (Planned/Actual per ticker + grand total + completion) from `dca_plans`+`dca_plan_items` for the month and `MailApp.sendEmail`s it to chanika.cptk@gmail.com, then flags every plan for the month `completed`. Backs the DCA page's "Complete Month" button. Params `userId`, `month` (`YYYY-MM`) |
...
dca           DCA Plan — multi-portfolio manual planner. Month selector + portfolio tabs; per tab: budget input + 🔄 Refresh (gap-from-target suggestions), a TICKER/Reasoning/Suggested/Planned/Actual/✓ table, status pill (DRAFT→IN PROGRESS→COMPLETED) + progress bar. "Complete Month" button (all tickers ticked across all portfolios) emails a summary via GAS `dcaEmailSummary`.
...
- `_dcaRefresh()` — ensures the plan, saves budget, runs `_dcaGenerate(portId, budget)` (holdings vs target: `currentPct=value/totalValue`, `gap=target-current`, keep `gap>0`, distribute budget ∝ gap, sorted; Suggested is non-editable), then `_dcaSyncItems(planId, generated)` **rebuilds Suggested/reasoning while preserving Planned/Actual/Done per ticker** (updates matches, keeps worked-but-no-longer-underweight rows with suggested 0, deletes empty stale rows, inserts new).
...
- `_dcaUpdateCompleteBtn()` — queries all of the month's plans with embedded `dca_plan_items(is_done)`; shows the **Complete Month** button only when every ticker across all portfolios is ticked. `_dcaCompleteMonth()` → `callGAS('dcaEmailSummary', {userId, month})` → emails the summary + marks plans completed, then reloads.
