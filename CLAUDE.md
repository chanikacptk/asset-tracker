...
> **Mobile stale US-stock price — SW was caching GAS live-price calls (2026-07-20)**: US stock prices on the **iPhone PWA didn't match Chrome laptop**, and the documented "force-close + reopen 2×" didn't help. **Root cause: the service worker (`sw.js`) fetch handler caches GAS API responses cache-first.** It has three branches — network-first for `*.supabase.co`, network-first for the app shell (`/`, `index.html`), and a **catch-all cache-first block for everything else**. `callGAS(...)` hits `https://script.google.com/macros/.../exec` (batched `getPrices` for live prices), whose host matches **neither** the Supabase nor app-shell exception → it fell through to **cache-first**. The first `getPrices?tickers=…` response was cached and **frozen per device** (each PWA/browser has its own Cache Storage, populated at different times → prices diverge between devices). force-close+reopen only refreshes the SW *version* (the `activate` handler deletes caches whose key ≠ current version); **same-version entries survive**, so the stale `getPrices` snapshot stayed. Supabase got a network-first exception here; **GAS was simply never added** — that was the leak. **NOT the cause** (ruled out): (a) mobile/desktop use the **same** `_renderHoldingsView` path — the mobile Dime card's price is `r.price` = `getLatestPrice`, identical to desktop, no mobile-only branch that skips live-refresh; (b) not a wrong-field display (`_usCardSec` price/day/pl all read the same `r.price`/`r.dayPct`); (c) not the old 2×N throttle (already batched into one `getPrices`). **Fix (frontend + SW only, no GAS change)**: **(1)** `sw.js` — added a **network-first branch for the GAS backend** (`url.hostname.includes('script.google') || 'googleusercontent.com'`, GAS `exec` 302-redirects to `script.googleusercontent.com`) mirroring the Supabase exception, placed **before** the catch-all; **(2)** `index.html` `callGAS` — its `fetch` now uses **`{ cache: 'no-store' }`** to also bypass the iOS Safari HTTP cache (a second layer needed on iOS). Both layers matter — SW Cache Storage + browser HTTP cache. **General rule**: any new backend host (another API/proxy/exec URL) must get an explicit network-first branch in `sw.js` before the catch-all — never rely on the default. When "data differs between devices but the DB is the same," suspect this SW cache layer, not the DB/GAS source (the `debug-price-fetch.md` DB checklist won't catch it). No migration, **no GAS republish** (nothing in `gas/` changed). SW cache `myasset-v117` → **`myasset-v118`**. **Deployed 2026-07-20** — pushed to `main` (commit `5ce188f`), GitHub Pages build+deploy succeeded (55s), live `sw.js` = `myasset-v118` confirmed. (iPhone PWA still needs the usual close+reopen 2× to install→serve the new SW.)

> **Cost optimization — Portfolio Analyze is now on-demand (no auto schedule) (2026-07-16)**: Same pattern applied to Portfolio Analyze as the news brief. Previously `onDailyTrigger` ran `AnalystAgent.reviewAllPortfolios()` (**`claude-sonnet-4-6`**, ~208k input tokens/day) automatically every weekday **and** via `onWeeklyTrigger` on Mondays — but with Telegram paused (`Config.TELEGRAM_ENABLED=false`) nobody ever saw the result. Now it only runs when the user taps a button. **Root-cause note on the "weekend Sonnet calls (7/4 Sat, 7/5 Sun)"**: `onDailyTrigger` (`Code.gs`) *does* gate the daily analysis with `dayOfWeek = today.getDay()` (evaluated in **Asia/Bangkok** per `appsscript.json` `timeZone`) inside `if (dayOfWeek >= 1 && dayOfWeek <= 5)`, so `reviewAllPortfolios` was **correctly skipped on Sat/Sun**. Also `AnalystAgent._callClaude` uses **no `web_search`** (plain Sonnet completion). So the weekend `web_search` cost was **not** from Portfolio Analyze — it matches the **old daily 7AM news brief** (`onNewsBriefTrigger` → `_callClaudeWebSearch`), which at that time was Sonnet+`web_search` and scheduled `.everyDays(1).atHour(7)` with **no weekday guard** → ran every day incl. weekends (removed + moved to Haiku/on-demand on the same 2026-07-16). **Changes: (1)** removed `AnalystAgent.reviewAllPortfolios()` from **both** `onDailyTrigger` (weekday branch) and `onWeeklyTrigger` in `gas/Code.gs` — no auto schedule remains. **(2)** new web action **`generatePortfolioAnalysis`** (param `portfolioId`: blank = all portfolios, set = that one) → `AnalystAgent.generatePortfolioAnalysis(portfolioId)`; the review fns now **return** a per-portfolio summary (`_analyzePortfolio` returns `{portfolioId,name,tickers,analyzedAt}` / `null`; skips push `{…,error}`), and the action returns `{ok, generatedAt, portfolios:[...]}`. **Persistence unchanged** — still writes `ai_analyses` (+ `created_at`) and `sr_levels`, no schema change. **(3)** Frontend **All Portfolio** + **Weekly Review** pages each show a shared bar `_analysisBar(lastIso)` = "🕐 วิเคราะห์ล่าสุด: X" (from the newest `ai_analyses.created_at`, `_analysisLastLabel` relative+abs Thai) + a **🔄 Refresh Analysis** button → `refreshPortfolioAnalysis()` → `callGAS('generatePortfolioAnalysis')` → loading state, success/error toast, then `loadPage(state.currentPage)` re-render — exact same UX as the Analysis › News **Generate Today's Brief** button. The old Weekly Review static **📆 Run Weekly Review** card (`runWeeklyReview`/`analyzeAll`/`weekly-run-btn`/`weekly-status`) was **removed** and merged into the single Refresh Analysis button (no duplicate). The per-holding **🤖 Analyze** button (US Portfolio → `analyzePortfolio` → `reviewPortfolioById`) is untouched and still works. **Untouched**: `DCAAgent.gs`, `Code.gs` `_claudeEarnings`, `onRealtimeTrigger`, `onMFNavTrigger`. The legacy `analyzeAll` web action still exists but is now unused by the app. SW cache `myasset-v117`. **Frontend deployed 2026-07-16** — pushed to `main` (commit `13d3bf2`), GitHub Pages run succeeded, live `sw.js` = `myasset-v117` and `generatePortfolioAnalysis` confirmed in the served `index.html`. **⚠️ GAS side STILL PENDING (not auto-deployed): paste `gas/AnalystAgent.gs` + `gas/Code.gs`, republish the Web App (Manage deployments ▸ Edit ▸ New version), re-run `setupTriggers()` to drop the live weekday `onDailyTrigger` schedule's analysis call (the trigger itself stays for fetch/news; only the code it runs changed).** Until the republish, the 🔄 Refresh Analysis button returns `Unknown action: generatePortfolioAnalysis`.

> **Cost optimization — Haiku news brief, Telegram paused, on-demand news (2026-07-16)**: Four changes to cut Claude/Telegram cost and API noise. **(1) News brief → Haiku** — `NotificationAgent.gs` `_callClaudeWebSearch` (the daily Tech-News brief) now uses **`claude-haiku-4-5`** instead of `claude-sonnet-4-6`. **Sonnet is deliberately kept** for the three real-analysis paths: `AnalystAgent.gs` (Portfolio Analyze), `DCAAgent.gs` (DCA plan), and `Code.gs` `_claudeEarnings` (Ticker Analysis earnings fallback). *(Note: the task originally asked to also switch a "Market Wrap" brief and to edit `DataAgent.gs`/`NewsAgent.gs` — but no market-wrap feature exists, and `NewsAgent.gs` uses NewsAPI not Claude; the only Claude-powered brief is the news brief.)* **(2) ALL Telegram sends paused** — new master switch **`Config.TELEGRAM_ENABLED: () => false`** in `gas/Config.gs`; both internal senders `_send` **and** `_sendHtml` guard with `if (!Config.TELEGRAM_ENABLED()) return;` at the top, so *every* automated notification (news brief, growth/weekly reviews, high-impact + realtime alerts) is silently paused **without deleting any code** — flip to `true` to re-enable everything. The manual **Test Telegram** button (`Code.gs` `_sendTestTelegram`, its own UrlFetchApp path) is intentionally **left working** for re-enable verification. **(3) 7AM news trigger removed** — the `onNewsBriefTrigger` block was deleted from `setupTriggers()` (must re-run `setupTriggers()` in the IDE to drop the live trigger; the function itself still exists for manual runs). **(4) On-demand news in the app** — new GAS web action **`generateNewsBrief`** (param `userId`) → **`NotificationAgent.generateNewsBriefForUser(userId)`** (generates + persists to `daily_news`, **no Telegram**, returns `{ok, date, holdings, market}`; exported from the IIFE). Frontend **Analysis › News** tab gains a full-width button (`#an-gen-wrap`/`.an-gen-btn`, rendered by `_anRenderGenBtn(hasToday)` inside `loadAnalysis`): **"🔄 Generate Today's Brief"** when today has no brief → **"✅ Generated today — Refresh"** once it does; `generateTodaysBrief()` calls `callGAS('generateNewsBrief', {userId})`, then `loadAnalysis()` re-renders. So the Claude API is only hit when the user actually wants to read the news. **Deploy gotcha hit & resolved**: re-running `setupTriggers()` proved the pasted Code.gs was in the IDE, but the Web App still returned `Unknown action: generateNewsBrief` until the **existing deployment** was republished (Manage deployments ▸ Edit ▸ New version) — the classic frozen-snapshot trap. **Verified end-to-end 2026-07-16** on the live exec URL: `ping` ok → action recognized (`userId required`) → full run `brief.ok:true` (4 holdings + 6 market) → **10 rows persisted** to `asset_track.daily_news` for today, no Telegram sent. SW cache `myasset-v116`. **⚠️ Requires: paste `gas/Config.gs` + `gas/NotificationAgent.gs` + `gas/Code.gs`, republish the Web App (New version), re-run `setupTriggers()`** — all done.

> **Merged into the MyExp+ Supabase project under an `asset_track` schema (2026-07-12)**: To stay under Supabase's free-tier 2-project limit, the whole asset-tracker backend was moved off its own project (`zchwqmykjjjtoaymuvwx`) into the shared **MyExp+** project (`evowtnrqlsdddclngsrv`, PG 17.6) under a **new `asset_track` Postgres schema** (MyExp+'s own `public` schema is untouched). All 28 tables + ~9,057 rows + 69 RLS policies were `pg_dump`ed, rewritten `public.` → `asset_track.`, and imported (row-count parity verified). **Two cross-project gotchas handled during import**: (a) `ALTER DEFAULT PRIVILEGES FOR ROLE supabase_admin` lines fail as the `postgres` role → commented out; (b) the `users.auth_uid → auth.users` FK was dropped (old Google identities don't exist in MyExp+'s `auth.users`) — the column + values stay, and the app **re-links by email** on next Google login. **How the schema is selected** (PostgREST needs telling — tables aren't in `public` anymore): frontend `index.html` sets `{ db: { schema: 'asset_track' } }` on `supabase.createClient` (routes all `.from()` calls); `gas/Config.gs` `supabaseHeaders()` sends `Accept-Profile: asset_track` (reads) + `Content-Profile: asset_track` (writes). **Config changes**: `index.html` `SUPABASE_URL`/`SUPABASE_ANON_KEY` → the MyExp+ project's URL + anon key; GAS Script Property `SUPABASE_URL` + `SUPABASE_SERVICE_KEY` → MyExp+'s. **Auth infra also re-pointed**: Google Cloud redirect URI → `https://evowtnrqlsdddclngsrv.supabase.co/auth/v1/callback`, and MyExp+'s **Auth → URL Configuration → Redirect URLs** must include `https://chanikacptk.github.io/asset-tracker/**` (missing it bounced Google login to MyExp+'s Site URL — the expense tracker). Verified end-to-end: app reads/writes, GAS wrote a fresh gold row to `asset_track.market_data`, Google sign-in re-linked Chelsea's `auth_uid` by email. Backups in `~/supabase-migration-backups/2026-07-12/`. SW cache `myasset-v115`. **The old project `zchwqmykjjjtoaymuvwx` is kept as a fallback and deleted manually by the user later.** See **Authentication** + **Script Properties**.

> **"GAS call failed: Failed to fetch" — was a duplicate `const` across `.gs` files + `callGAS` masking the real error (2026-07-11)**: Every GAS action broke app-wide with "Failed to fetch". **Root cause: GAS compiles ALL `.gs` files as one shared global scope**, so a `const DCAAgent` accidentally pasted into `DataAgent.gs` (in addition to the real one in `DCAAgent.gs`) threw a project-wide `SyntaxError: Identifier 'DCAAgent' has already been declared` → the Web App returned an **HTML error page** (HTTP 200) for *every* request → the frontend's `resp.json()` choked and surfaced a generic "Failed to fetch", hiding the real cause. **Diagnosis that worked**: hit `‹exec-url›?action=ping` directly (curl/browser) — it returned the GAS HTML error page naming the duplicate identifier + file. **The duplicate was NOT in the repo** (repo `gas/` was clean — `grep -nE '(const|var|let)\s+DCAAgent' *.gs` showed exactly one); it existed only in the deployed IDE copy, so fixing it = deleting the stray declaration in the IDE and redeploying. **Gotchas hit along the way**: (a) the error said "line 1, file DCAAgent" but the real file declares it at line 6 — misleading; the actual dup was in a *different* file; (b) a duplicate can hide in ANY `.gs` file, and even a mis-named double-extension file like `Config.gs.gs` compiles into scope; (c) "redeploy" only takes effect if you pick **Version → New version** (saving or re-deploying the existing version serves the old frozen snapshot); a *New deployment* mints a new URL and snapshots current HEAD (good for proving current code compiles). **To debug a `const X already declared`**: `grep -rnE '(const|var|let)\s+X\b' gas/*.gs` must return exactly ONE line across ALL files. **Frontend hardening (shipped)**: `callGAS` now reads the response as **text**, runs `_parseGasHtmlError(text)` (pulls the message from GAS's `font-family:monospace` error `<div>`, decodes entities; returns null for real JSON) and throws the real Apps Script error instead of "Unexpected token <"; adds **3× retry w/ 1s backoff** on network failure (no retry on script errors — broken code won't fix itself) + an actionable Thai toast `⚠️ ไม่สามารถเชื่อมต่อ GAS ได้ …`. New **Test Connection** button in Settings (`testGasConnection`) pings `?action=ping` and shows ✅ connected + response-time / ❌ exact failure (works on the typed URL before saving). i18n `set.testGas` (EN/TH). SW cache `myasset-v114`. No GAS/migration change on the repo side.
...
| Database | Supabase (PostgreSQL + REST API) — the **MyExp+** project (`evowtnrqlsdddclngsrv`), tables under the **`asset_track`** schema (migrated 2026-07-12). Select the schema via `{ db: { schema: 'asset_track' } }` (frontend) / `Accept-Profile`+`Content-Profile` headers (GAS) |
| Backend | Google Apps Script (GAS) — `.gs` files in `gas/` |
| AI | Claude API called from GAS — **`claude-sonnet-4-6`** for portfolio analysis / DCA / ticker earnings; **`claude-haiku-4-5`** for the daily news brief (cost, 2026-07-16) |
| Notifications | Telegram bot (per-user chat IDs) |
| PWA | `manifest.json` + `sw.js` (cache `myasset-v118`) |
...
> **Since 2026-07-12** the app points at the shared **MyExp+** project (`evowtnrqlsdddclngsrv`) and its tables live in the **`asset_track`** schema, not `public`. The Google-auth redirect infra below was re-pointed to that project; the Google Cloud redirect URI is now `https://evowtnrqlsdddclngsrv.supabase.co/auth/v1/callback` and the `**` app-redirect allowlist entry lives in *MyExp+'s* Auth → URL Configuration. See the migration changelog block up top.
...
| Property | Value |
|---|---|
| `SUPABASE_URL` | `https://evowtnrqlsdddclngsrv.supabase.co` *(MyExp+ project — tables under `asset_track` schema, selected via `Accept-Profile`/`Content-Profile` in `Config.gs`)* |
...
- ~~Daily @ 7AM → `onNewsBriefTrigger`~~ **removed 2026-07-16** — the news brief is now generated **on demand** from the app (Analysis › News button → web action `generateNewsBrief`). `onNewsBriefTrigger()` still exists for manual runs; Telegram sends are paused (`Config.TELEGRAM_ENABLED=false`).
- Daily @ 8AM → `onDailyTrigger` (weekdays: fetch → news → notify; monthly on day 1; weekly on Monday). **Portfolio analysis (`reviewAllPortfolios`) is NO LONGER run here — on-demand only via `generatePortfolioAnalysis`, 2026-07-16.** Re-run `setupTriggers()` after pasting so the live trigger runs the updated code.
...
| `analyzeAll` | AnalystAgent.reviewAllPortfolios() — **legacy, no longer called by the app** (superseded by `generatePortfolioAnalysis`); kept for manual runs |
| `analyzePortfolio` | AnalystAgent.reviewPortfolioById(portfolioId) — backs the US Portfolio per-tab **🤖 Analyze** button (persists to `ai_analyses`/`sr_levels`) |
| `generatePortfolioAnalysis` | **On-demand portfolio analysis (2026-07-16)** — param `portfolioId` (blank = all portfolios, set = that one) → `AnalystAgent.generatePortfolioAnalysis(portfolioId)` runs the Sonnet review + **persists** to `ai_analyses`/`sr_levels` (no schema change), returns `{ok, generatedAt, portfolios:[{portfolioId,name,tickers,analyzedAt}\|{…,error}]}`. Backs the **All Portfolio** + **Weekly Review** pages' **🔄 Refresh Analysis** button (`refreshPortfolioAnalysis` → `_analysisBar`); the Claude API is only hit when the user taps it (replaced the auto `onDailyTrigger`/`onWeeklyTrigger` call) |
...
| `sendNewsBrief` | NotificationAgent.sendDailyNewsBrief() — generates + (would) send the daily holdings-aware Tech-News brief for all users. **Telegram is now paused** (`Config.TELEGRAM_ENABLED=false`) so this only persists to `daily_news`; kept for manual testing |
| `generateNewsBrief` | **On-demand brief for ONE user (2026-07-16)** — param `userId`; `NotificationAgent.generateNewsBriefForUser(userId)` runs the Haiku `web_search` call, **persists** to `daily_news`/`daily_news_impact`, and returns `result.brief = {ok, date, holdings, market}` — **no Telegram**. Backs the Analysis › News **Generate Today's Brief** button; the Claude API is only hit when the user taps it (replaced the auto 7AM trigger) |
...
weekly        Weekly Review — AI signals (last 7 days) + a "🕐 วิเคราะห์ล่าสุด: X" bar with a 🔄 Refresh Analysis button → generatePortfolioAnalysis (on-demand; replaced the old 📆 Run Weekly Review/analyzeAll button)
allportfolio  All Portfolio — read-only AI signals across all holdings + the same 🕐 last-analyzed bar + 🔄 Refresh Analysis button (generatePortfolioAnalysis)
...
Cache name: **`myasset-v118`**. Bump on every `index.html` change.

Strategy:
- Network-first: Supabase API, **GAS backend (`script.google` / `googleusercontent.com`)**, `index.html` / app root (ensures updates + live prices always show)
- Cache-first: CDN assets (Chart.js, Supabase JS)
- Precached: CDN bundles only (not the app shell)

> **Any new backend host must get its own network-first branch here, before the catch-all cache-first block** — otherwise its API responses get cached-first and freeze per device (this caused the 2026-07-20 "iPhone price ≠ Chrome" bug: GAS `getPrices` was falling through to cache-first). See the top-of-file 2026-07-20 changelog entry.
